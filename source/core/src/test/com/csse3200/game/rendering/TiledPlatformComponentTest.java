...
import static org.mockito.Mockito.*;
  ...
  class TiledPlatformComponentTest {
    ...
    // Standard 16x16 pixel tiles, giving aspect ratio of 1.0
    // This is same as the size of out platfom tiled textures
    ...
    void shouldDrawHalvesForPlatformLessThanTwoTilesWide() {
      entity.setScale(1.5f, 1f);
      TiledPlatformComponent component = new TiledPlatformComponent(leftEdge, middleTile, rightEdge);
      component.setEntity(entity);

      component.draw(spriteBatch);

      verify(spriteBatch).draw(any(TextureRegion.class), eq(0f), eq(0f), eq(0.75f), eq(1f));
      verify(spriteBatch).draw(any(TextureRegion.class), eq(0.75f), eq(0f), eq(0.75f), eq(1f));
    ...
    void shouldDrawOnlyEdgesForTwoTileWidePlatform() {
      ...
      inOrder.verify(spriteBatch).draw(eq(leftEdge), eq(0f), eq(0f), eq(1f), eq(1f));
      inOrder.verify(spriteBatch).draw(eq(rightEdge), eq(1f), eq(0f), eq(1f), eq(1f));
      verifyNoMoreInteractions(spriteBatch);
    }

    @Test
    void shouldDrawStretchedTilesForNonIntegerWidthPlatform() {
      entity.setScale(3.5f, 1f);
      TiledPlatformComponent component = new TiledPlatformComponent(leftEdge, middleTile, rightEdge);
      component.setEntity(entity);

      component.draw(spriteBatch);

      float stretchedWidth = 3.5f / 4f;

      InOrder inOrder = inOrder(spriteBatch);
      inOrder.verify(spriteBatch).draw(eq(leftEdge), eq(0f), eq(0f), eq(stretchedWidth), eq(1f));
      inOrder.verify(spriteBatch).draw(eq(middleTile), eq(stretchedWidth), eq(0f), eq(stretchedWidth), eq(1f));
      inOrder.verify(spriteBatch).draw(eq(middleTile), eq(stretchedWidth * 2), eq(0f), eq(stretchedWidth), eq(1f));
      inOrder.verify(spriteBatch).draw(eq(rightEdge), eq(stretchedWidth * 3), eq(0f), eq(stretchedWidth), eq(1f));
      verifyNoMoreInteractions(spriteBatch);
    }

    @Test
    void shouldDrawCorrectNumberOfMiddleTiles() {
      entity.setScale(10f, 1f);
      TiledPlatformComponent component = new TiledPlatformComponent(leftEdge, middleTile, rightEdge);
      component.setEntity(entity);
    ...
    void shouldHandleDifferentAspectRatio() {
      ...
      entity.setScale(5f, 1f);
      TiledPlatformComponent component = new TiledPlatformComponent(wideLeft, wideMiddle, wideRight);
      component.setEntity(entity);

      component.draw(spriteBatch);
      float stretchedWidth = 5f / 3f;

      InOrder inOrder = inOrder(spriteBatch);
      inOrder.verify(spriteBatch).draw(eq(wideLeft), eq(0f), eq(0f), eq(stretchedWidth), eq(1f));
      inOrder.verify(spriteBatch).draw(eq(wideMiddle), eq(stretchedWidth), eq(0f), eq(stretchedWidth), eq(1f));
      inOrder.verify(spriteBatch).draw(eq(wideRight), eq(stretchedWidth * 2), eq(0f), eq(stretchedWidth), eq(1f));
      verifyNoMoreInteractions(spriteBatch);
    }
    ...
    void shouldThrowAssertionErrorForMismatchedTileDimensions() {
      ...
      assertThrows(AssertionError.class, () -> {
        new TiledPlatformComponent(leftEdge, mismatchedMiddle, rightEdge);
      });
    }
  }